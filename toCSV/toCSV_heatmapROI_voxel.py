# ******************************************* #
#
# Module used to transform parsed .txt data to .csv used in learning notebook.
# This module incorporates both the ROI from the Heatmap and the voxelized point cloud.
# This is module currently in use to generate training data.
#
# ******************************************* #
...
#
# Func to search for a peak in a window around the given range,
...
#
# Func to get the ROI data given the voxelized point cloud information
#
def get_ROI(processed_heatmap, rangeIdx, size_x=16, size_y=16, skip_num=5):  # size=16, skip_num=5):
    ...
    ROI[ROIstart_x:ROIend_x:, ROIstart_y:ROIend_y:] = processed_heatmap[data_start_x:data_end_x:,
...
#
# Func to give the mean range of the detected objects, used to generate ROI:
#
def mean_range(object_list):
    mean = 0
    for i in range(len(object_list)):
        mean += objects[i]["rangeIdx"]
...
if __name__ == "__main__":
    ...
    f_cfg = open('test_crop/knife3dNoGrouping.cfg', "r")
    ...
    with open(target_folder + "/mmwaveRA.csv", mode="w", newline='') as RA:
        ...
        for folder in sub_folders:
            ...
            if (not ".DS" in folder) and (not ".dat" in folder) and(not "hand" in folder):
                ...
                for name in txts:
                    ...
                    if (not ".DS" in curr_name):
                        ...
                        objects, max_x, max_y, max_z, min_x, min_y, min_z = read_detected_points(curr_name)
                        # # get the label:
                        # label = get_label(folder)
                        # cluster the points :
                        if objects is None:
                            ...
                            grouped_objects = groupPoints(objects)
                        ...
                        if grouped_objects is None or not cluster_ok(grouped_objects,max_x,max_y,max_z,min_x,min_y,min_z):
                            ...
                            print(max_x > min_x)
                            # Get the range idx to look for the ROI
                            detected_point_range = mean_range(objects)
                            # extract the RA heatmap (in polar coordinates )
                            processed_heatmap = process_RA(dat, range_bin, ang_bin)
                            # Exctract ROI, size =16 - gives 32x32 window
                            ROI = get_ROI(processed_heatmap, detected_point_range, size_x=16,
                                          size_y=16)  # 16 - gives 32x32

                            #### Uncomment If want to include Interpolation using Convex Hull
                            # pts, pts_with_val = toList(grouped_objects)
                            # hull = ConvexHull(pts)
                            # grouped_objects = sampleHull(hull, pts_with_val, grouped_objects)

                            max_x, max_y, max_z, min_x, min_y, min_z = getMinMax(grouped_objects)


                            ####
                            # Transform the pointcloud to voxels:
                            voxel = toVoxel(grouped_objects, 25, max_x, max_y, max_z, min_x, min_y, min_z)

                            # WRITING DATA :

                            # Save the heatmap ROI
                            np.savetxt(heatmap_id + "_heatmap.csv", ROI, delimiter=",")
                            # Save the voxel
                            voxel = voxel.reshape(26, 26 * 26)
                            np.savetxt(heatmap_id + "_voxel.csv", voxel, delimiter=",")
                            # And add the reference to the file
                            RA_writer.writerow([folder + "_" + name[:-4:], label])
                            # plt.imsave("del/" + heatmap_id[30::] + ".png", ROI)
