# ******************************************* #
#
# Module used to parse the configuration file and obtain the sensor parameters from .cfg
# Adapted from pymmw Mmwave module (https://github.com/m6c7l/pymmw)
#
# ******************************************* #
...
def range_resolution(cfg):
    return (300 * cfg['profileCfg']['sampleRate']) / (
                cfg['profileCfg']['adcSamples'] * 2 * cfg['profileCfg']['freqSlope'] * 1e3)


def doppler_resolution(cfg):
    return 3e8 / (2 * cfg['profileCfg']['startFreq'] * 1e9 * (
                cfg['profileCfg']['idleTime'] + cfg['profileCfg']['rampEndTime']) * 1e-6 * chirps_per_frame(cfg))


if __name__ == "__main__":
    f = open("profile.cfg", "r")
    if len(sys.argv) < 2:
        tx_ant, rx_ant = 2, 4
        print("_______")
        print(
            "Note : Tx and Rx antennas not specified, used 4,2. If you wish to specify, use syntax parseCFG.py <tx_ant> <rx_ant>")
        print("_______")
    else:
        tx_ant, rx_ant = sys.argv[1], sys.argv[2]

    cfg = parseCFG(f, tx_ant, rx_ant)

    ang_bins, doppler_bin, range_bin = num_angular_bin(cfg), num_doppler_bin(cfg), num_range_bin(cfg)
    range_res, doppler_res = range_resolution(cfg), doppler_resolution(cfg)
